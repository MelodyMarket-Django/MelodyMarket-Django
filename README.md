...
## 목표와 기능
### 목표 ###
Melody market은 음악 스트리밍 사이트로, 구독 기반의 서비스 제공합니다. 계정 관리, 음악 검색 및 추천, 플레이리스트 관리 및 노래에 대한 리뷰 작성 기능을 제공하여 사용자들이 개인화된 음악 스트리밍 경험을 제공하는 것을 목표로 합니다.


### 기능
- **회원가입과 로그인/로그아웃:** 사용자는 이메일과 비밀번호를 이용해 계정을 생성하고 로그인 및 로그아웃을 할 수 있습니다.
- **노래 검색:** 모든 사용자는 사이트에서 원하는 노래를 검색할 수 있습니다. 스포티파이 API를 사용하여 검색 결과를 반환합니다.
- **구독 관리 및 결제 처리:** 사용자는 음악 구독 옵션을 선택하고, 구독을 취소하거나 정보를 조회할 수 있습니다. 외부 결제 API를 사용하여 사용자는 신용카드 정보를 입력하여 결제를 진행하며 결제 완료 기능을 지원합니다.
- **플레이리스트 관리:** 사용자는 플레이리스트를 생성, 수정, 삭제하고 노래를 추가하거나 제거할 수 있습니다.
- **리뷰 관리:** 사용자는 음악 리뷰를 작성, 조회하고 필요에 따라 수정하거나 삭제할 수 있습니다.
...
## 개발하며 느낀점 
**선하**:
개발자는 단순히 코드 한 줄 한 줄을 작성하는 것이 아니라, 프로젝트 전체를 파악하고, 각 부분이 어떻게 조화롭게 작동하는지 이해하는 것이 중요하다는 것을 깨달았습니다. 또한, 각 부분을 개별적으로 다루면서도 전체적인 목표를 고려해야 한다는 균형을 유지해야 한다고 생각했습니다. 
**진선**:
...
**찬수** : 
개발 과정을 마치고 나니, 시간이 더 있었으면 하는 바람이 강하게 듭니다. 프로젝트 초기 단계에서 계획한 일정에 비해 실제 개발에 필요한 시간을 과소평가한 것 같아, 추가적인 기능 개선과 버그 수정을 위한 여유가 부족했습니다. 특히 복잡한 기능들의 통합과 최종 테스트 단계에서 더 많은 시간을 할애할 수 있었다면, 제품의 품질을 한층 더 높일 수 있었을 것입니다. 앞으로는 보다 현실적인 일정 계획과 함께, 예상치 못한 오류에도 대비할 수 있는 여유 시간을 더 확보하는 방향으로 접근하면 좋았을 듯 합니다.
